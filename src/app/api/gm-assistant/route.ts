...
import { getFuelCapacity } from "@/lib/game/items";
import {
...
export async function POST(req: Request) {
  ...
  const rosterLines = roster
    .map((c) => {
      ...
      const fuelMax = getFuelCapacity(c.equipment ?? []);
      const fuel = fuelMax > 0 ? `${c.fuel ?? fuelMax}/${fuelMax}` : "n/a (no fuel-fed gear)";
      ...
      return `- id: ${c.id} | name: ${c.name} | class: ${c.className ?? "unknown"} (classId: ${
        ...
        c.maxHp ?? 10
      } | mutation: ${c.mutationLevel ?? 0} | humanity: ${c.humanity ?? 10} | soul charges: ${soulCharges} | fuel: ${fuel} | equipment: ${equipment} | installed grafts: ${grafts} | unlocked graft ids: ${unlocked} | known blueprint ids: ${knownBlueprints} | pending item use: ${pendingUse}`;
    })
    .join("\n");
